<aside x-data="{ open: true }" class="flex h-screen flex-col bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-200 p-4 overflow-y-auto transition-all duration-300" :class="open ? 'w-64' : 'w-20'">

    {{-- Bagian Logo dan Nama Aplikasi --}}
    <div class="flex items-center mb-8 text-center">
        <a href="{{ route('dashboard') }}">
            <span class="text-2xl font-bold text-blue-900 text-center" :class="{ 'hidden': !open }">Manajemen Keuangan</span>
            <svg :class="{ 'hidden': open }" class="h-8 w-8 text-blue-900" fill="currentColor" viewBox="0 0 24 24">
    ...
    <button @click="open = !open" class="absolute top-5 left-4 bg-white dark:bg-gray-800 p-1 rounded-full shadow-md">
        ...
        @foreach ($navigation as $item)
            @if(!isset($item['can']) || (isset($item['can']) && auth()->user()->role->value === $item['can']))
                ...
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
                    ...
                    {!! $item['icon'] !!}
                    <span class="text-sm" :class="{ 'hidden': !open }">{{ $item['name'] }}</span>
                </a>
            @endif
        @endforeach
    </nav>
    ...
    <div class="mt-auto flex flex-col gap-2">
        ...
        class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
            ...
            <span class="text-sm" :class="{ 'hidden': !open }">Pengaturan</span>
        </a>
    </div>
</aside>
