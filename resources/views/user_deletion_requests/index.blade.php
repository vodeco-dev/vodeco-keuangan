<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">
            {{ __('Riwayat Permintaan Penghapusan') }}
        </h2>
    </x-slot>
                                ...
                                @forelse ($requests as $request)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $request->transaction->description ?? 'Tanpa Deskripsi' }}</div>
                                            <div class="text-sm text-gray-500">Rp {{ number_format($request->transaction->amount, 0, ',', '.') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $request->created_at->format('d F Y H:i') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
